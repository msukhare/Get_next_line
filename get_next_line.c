...
/*   Updated: 2017/11/27 14:08:17 by msukhare         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int			ft_readfile(t_gnl *lst, const int fd)
{
	...
	if (!(temp = (char *)ft_memalloc(BUFF_SIZE + 1)))
		return (-1);
	if (!(lst->p_line = (char *)ft_memalloc(1)))
		return (-1);
	while ((file = read(fd, temp, BUFF_SIZE)) > 0)
	{
		to_free = lst->p_line;
...
static int			ft_cpy(t_gnl *temp, char **line)
{
	int				nb_cara;
	int				help;
...
int					get_next_line(const int fd, char **line)
{
	...
	if (!line || BUFF_SIZE < 1 || read(fd, 0, 0) < 0)
		return (-1);
	if (!begin_list)
	{
